...
if(cluster.isMaster) {
    for( var i=0; i<numbers; i++) {
        console.log('master: about to fork a master ');
        cluster.fork();
    }
    ...
    http.createServer(function (req, res) {
        ...
        res.end('hello world from worker #' + cluster.worker.id + 'pid ' + cluster.worker.id + '(pid ' + cluster.worker.process.pid + ') with count = ' + count + '\n');
        if (count === 3) {
